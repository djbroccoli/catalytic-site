...
<header>
        ...
        <!-- Indicators -->
            <ol class="carousel-indicators">
                ...
                $sql = "SELECT * FROM full_db WHERE FIND_IN_SET($artistid,artistid) > 0 AND (newadd = 'x' OR newrelease = 'x') AND act = 1 LIMIT $limit OFFSET 4";
                $result = $conn->query($sql);
                ...
                <div class="item active">
                    <div class="container-fluid">
                        ...
                        //$limit = 4;
                                ...
                                $sql = "SELECT full_db.*, img_db.imgpath FROM full_db LEFT JOIN img_db ON full_db.coverimgid = img_db.id WHERE FIND_IN_SET($artistid,artistid) > 0 AND (newadd = 'x' OR newrelease = 'x') AND full_db.act = 1 ORDER BY releaseyear DESC LIMIT $limit";
                                $result = $conn->query($sql);
                       ...
                       <?php }
                        ...
                        //$limit = 4;
                                ...
                                $sql = "SELECT full_db.*, img_db.imgpath FROM full_db LEFT JOIN img_db ON full_db.coverimgid = img_db.id WHERE FIND_IN_SET($artistid,artistid) > 0 AND (newadd = 'x' OR newrelease = 'x') AND full_db.act = 1 ORDER BY releaseyear DESC LIMIT ".$limit." OFFSET 4";
                                $result = $conn->query($sql);
                                $rowcount = $result->num_rows;
    ...
    $start_from = ($page-1) * $limit;
            ...
            if($filter == 'solo') {
                ...
                $sql = "SELECT full_db.*, img_db.imgpath FROM full_db LEFT JOIN img_db ON full_db.coverimgid = img_db.id WHERE FIND_IN_SET($artistid,artistid) > 0 AND full_db.act = 1 AND albumartist LIKE '" .$artistname. "' ORDER BY albumartist ASC LIMIT $start_from, $limit";
            }
            elseif($filter == 'bands') {
                // query to get all Herring records
                $sql = "SELECT full_db.*, img_db.imgpath FROM full_db LEFT JOIN img_db ON full_db.coverimgid = img_db.id WHERE FIND_IN_SET($artistid,artistid) > 0 AND full_db.act = 1 AND albumartist NOT LIKE '" .$artistname. "' ORDER BY albumartist ASC LIMIT $start_from, $limit";
            } else {
                // query to get all records
                $sql = "SELECT full_db.*, img_db.imgpath FROM full_db LEFT JOIN img_db ON full_db.coverimgid = img_db.id WHERE FIND_IN_SET($artistid,artistid) > 0 AND full_db.act = 1 ORDER BY albumartist ASC LIMIT $start_from, $limit";
            }
   ...
   <!-- Catalog Section -->
    <section id="catalog" class="catalog-section">
        <div class="">
            ...
            if($filter == 'solo') {
                // query to get all Fitzgerald records
                $sql = "SELECT COUNT(id) FROM full_db WHERE full_db.act = 1 AND FIND_IN_SET($artistid,artistid) > 0 AND albumartist LIKE '".$artistname."'";
            }
            elseif($filter == 'bands') {
                // query to get all Herring records
                $sql = "SELECT COUNT(id) FROM full_db WHERE full_db.act = 1 AND FIND_IN_SET($artistid,artistid) > 0 AND albumartist NOT LIKE '".$artistname."'";
            } else {
                // query to get all records
                //$sql = "SELECT COUNT(id) FROM full_db WHERE full_db.act = 1 AND artistid = $artistid";
                $sql = "SELECT COUNT(id) FROM full_db WHERE full_db.act = 1 AND FIND_IN_SET($artistid,artistid) > 0";
            }

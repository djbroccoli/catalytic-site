<?php require_once 'basics/config.php';
    ...
    $savesql = "UPDATE full_db SET artistid=?, albumname=?, albumartist=?, personnel=?, label=?, releaseyear=?, formats=?, bandcamplink=?, quantity=?, wholesalecost=?, physical=?, albumcredits=?, coverlink=?, coverimgid=?, newadd=?, newrelease=?, dontslide=? WHERE id=?";
    ...
    if (!$stmt->bind_param('sssssssssssssssssi', $varmultipleartistpage, $varalbumname, $varartist, $varpersonnel, $varlabel, $varreleaseyear, $varformats, $varbandcamplink, $varquantity, $varwholesale, $varphysical, $vardesc, $varcoverlink, $varcoverimgid, $varnewadd, $varnewrelease, $vardontslide, $varid)) {

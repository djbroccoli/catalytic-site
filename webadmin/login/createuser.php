...
if ($pw1 != $pw2) {
    ...
    if (isset($_POST['newuser']) && isset($_POST['password1'])) {

        //Tries inserting into database and add response to variable

        $a = new NewUserForm;

        $response = $a->createUser($newuser, $newid, $oldemail, $newpw);
        ...
        if ($response == 'true') {
            ...
            $m = new MailSender;
            $responsem = $m->sendMail($newemail, $newuser, $newid, 'Verify');
            ...
            echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'. $signupthanks .'</div><div id="returnVal" style="display:none;">true</div>';
                ...
                echo '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>There was a problem. Please, contact your webmaster.</div><div id="returnVal" style="display:none;">true</div>';
                echo $newemail;
                echo $newuser;
                echo $newid;
                echo $responsem;

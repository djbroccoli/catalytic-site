    ...
    require_once 'webadmin/basics/conexion.php';
        ...
        if (!$stmt->execute()) {
         echo "<h3><i class='fa fa-exclamation-triangle fa-1x'></i> Something went wrong</h3><p>Please, try again later</p>";
            ...
            $cleaningsql = "UPDATE full_db SET aotw=? WHERE aotw=?";
            $cleaningstmt = $conn->prepare($cleaningsql);
            $cleaningstmt->bind_param('ss',$varcleaning,$varmark);
            $cleaningstmt->execute();

            $tagsql = "UPDATE full_db SET aotw=? WHERE id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=?";

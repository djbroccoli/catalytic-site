<?php
    ...
    require_once 'admin/basics/conexion.php';
		...
		$randsql = "SELECT * FROM full_db WHERE act = 1 AND releaseyear < '2016' AND (formats NOT LIKE '%T-Shirt%' OR formats NOT LIKE '%Poster%')  ORDER BY RAND() LIMIT 30";
        ...
        if (!$stmt->execute()) {
         echo "<h3><i class='fa fa-exclamation-triangle fa-1x'></i> Something went wrong</h3><p>Please, try again later</p>";
            ...
            if ($globalconfrow["sendaotw"] == 1) {
  	        ...
  	        $headers .= 'From: Catalytic Sound <albumoftheweek@catalytic-sound.com>' . "\r\n";

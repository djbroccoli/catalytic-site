<?php
    ...
    require_once 'webadmin/basics/conexion.php';
		...
		$randsql = "SELECT * FROM full_db WHERE act = 1 AND releaseyear < '2015' AND releaseyear NOT LIKE '' AND (formats NOT LIKE '%T-Shirt%' OR formats NOT LIKE '%Poster%') AND label NOT LIKE 'Trost' ORDER BY RAND() LIMIT 30";
        ...
        $verifiedsql = "INSERT INTO aotw_db_test (albumid, week) VALUES (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?), (?, ?)";
        ...
        if (!$stmt->execute()) {
         echo "<h3><i class='fa fa-exclamation-triangle fa-1x'></i> Something went wrong</h3><p>Please, try again later</p>";
            ...
            $cleaningsql = "UPDATE full_db_ctest SET aotw=? WHERE aotw=?";
            $cleaningstmt = $conn->prepare($cleaningsql);
            $cleaningstmt->bind_param('ss',$varcleaning,$varmark);
            $cleaningstmt->execute();

            $tagsql = "UPDATE full_db_ctest SET aotw=? WHERE id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=?";
            $tagstmt = $conn->prepare($tagsql);
            $tagstmt->bind_param('siiiiiiiiiiiiiiiiiiiiiiiiiiiiii',$varmark,
            $varalbumid[0],
            $varalbumid[1],
            $varalbumid[2],
            $varalbumid[3],

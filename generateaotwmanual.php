<?php
    ...
    require_once 'webadmin/basics/conexion.php';
		...
		$prevsql = "SELECT * FROM (SELECT * FROM aotw_db ORDER BY id DESC LIMIT 7) sub ORDER BY id ASC";

		$result = $conn->query($prevsql);
		...
		if ($result->num_rows > 0) {
			...
			//$rows = array();
            	...
            	while($prevrow = $result->fetch_assoc()) {
            		...
            		$varaotwid[] = $prevrow['id'];
            		$varalbumid[] = $prevrow['albumid'];
        ...
        $stmt->bind_param("isisisisisisis",
        $varalbumid[0],
        $date,
        $varalbumid[1],
        $date,
        ...
        if (!$stmt->execute()) {
         echo "<h3><i class='fa fa-exclamation-triangle fa-1x'></i> Something went wrong</h3><p>Please, try again later</p>";
            ...
            $tagsql = "UPDATE full_db SET aotw=? WHERE id=? OR id=? OR id=? OR id=? OR id=? OR id=? OR id=?";
            $tagstmt = $conn->prepare($tagsql);
            $tagstmt->bind_param('siiiiiii',$varmark,
            $varalbumid[0],
            $varalbumid[1],
            $varalbumid[2],
            $varalbumid[3],
